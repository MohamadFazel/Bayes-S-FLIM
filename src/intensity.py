...
def sample_int(
    ...
    lf_top = calculate_lifetime_likelihood_gpu_int(
        cp.asarray(i_new),
        cp.asarray(eta),
        cp.asarray(pi_bg),
        tau_irf,
    ...
    lf_bot = calculate_lifetime_likelihood_gpu_int(
        cp.asarray(i_old),
        cp.asarray(eta),
        cp.asarray(pi_bg),
        tau_irf,
    ...
    a_prior = np.sum(
        sc.gamma.logpdf(i_new[:, :], alpha_i, scale=beta_i), axis=0
    ) - np.sum(sc.gamma.logpdf(i_old[:, :], alpha_i, scale=beta_i), axis=0)
    a_prop = np.sum(
        sc.gamma.logpdf(i_old[:, :], alpha_prop, scale=(i_new[:, :] / alpha_prop)),
        axis=0,
    ) - np.sum(
        sc.gamma.logpdf(i_new[:, :], alpha_prop, scale=(i_old[:, :] / alpha_prop)),
        axis=0,
    )
