...
class FLIMAnalyzerApp(QWidget):
    ...
    def initUI(self):
        ...
        self.addParameter(
            "IRF Std:", "SigIRF", self.SigIRF, placeholder_color="#FFD700", row=1, col=0
        )
        self.addParameter(
            "Number of Species:",
            "num_species",
        ...
        self.addParameter(
            "Slice Parameters (e.g., 54:118, 0:64):",
            "slice_params",
            str(self.slice_params),
            placeholder_color="#FFD700",
            row=3,
            col=0,
            col_span=3,
    ...
    def runAnalysis(self):
        ...
        try:
            ...
            self.slice_params = tuple(
                slice(map(int, s.split(":"))) for s in slice_params_str.split(",")
            )
        except ValueError:
        ...
        for it in range(self.num_species):
            self.canvas.plotHistogram(
                1 / eta[-num_avg:, 0], bins=100, color="red", label="Viafluor"
            )
            self.canvas.draw()
