...
for it in range(pin.shape[0]):
    plt.plot(pin[it] / np.sum(pin[it]), plot_color[it], label=f"spectra {it+1}")
...
for eta_it in range(eta.shape[1]):
    plt.hist(1 / eta[-num:, eta_it], bins=100, color=plot_color[eta_it], label=f"eta {eta_it+1}")
plt.legend()
...
for phi_it in range(phi.shape[0]):
    ...
    plt.colorbar()
    plt.title(f"map {phi_it+1}")
    plt.savefig(f"{save_path}/map_{phi_it+1}.png")
    plt.show()
