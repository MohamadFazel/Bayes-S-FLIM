...
def sample_bg(pi_bg_old, photon_int, eta, tau_irf,  sig_irf, dt_padded, tiled_mask, t_inter_p, num, accept_bg):
    ...
    lf_top = calculate_lifetime_likelihood_gpu(cp.asarray(photon_int), cp.asarray(eta), cp.asarray(pi_bg_prop), tau_irf,  sig_irf, dt_padded, tiled_mask, t_inter_p, num)
    lf_bot = calculate_lifetime_likelihood_gpu(cp.asarray(photon_int), cp.asarray(eta), cp.asarray(pi_bg_old), tau_irf,  sig_irf, dt_padded, tiled_mask, t_inter_p, num)
